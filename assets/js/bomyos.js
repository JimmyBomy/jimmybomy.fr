...
const PROJECTS={
  amitie:{name:"Amitié Artisan",cat:"Client · réseau d'artisans du terroir",img:"assets/shots/amitie.jpg",grad:"linear-gradient(135deg,#1e2a44,#2b3b5e 55%,#c96f5a)",role:"Développement web",year:"2026",client:"Amitié Artisan — client, en ligne",tags:["Client","Web","Espace membres","En ligne"],link:"https://www.amitie-artisan.fr",desc:"Site du réseau Amitié Artisan : une plateforme qui met en relation les artisans et producteurs du terroir avec le public — présentation du réseau, avantages, formules d'abonnement et espace membres (inscription / connexion). Projet client, en ligne sur amitie-artisan.fr."},
  boulangerie:{name:"La Fournée de Sedan",cat:"Démo — site de commerce",img:"assets/shots/boulangerie.jpg?v=2",grad:"linear-gradient(135deg,#8a5a1c,#c8892e 55%,#f2d06b)",role:"Exemple pour commerçants & indépendants",year:"2026",client:"Démonstration",tags:["Site vitrine","Click & Collect","Responsive","Fait main"],link:"https://jimmybomy.fr/projets/demo-boulangerie/",desc:"Un exemple concret de ce que votre commerce pourrait avoir : site vitrine soigné, carte éditoriale, click & collect interactif, horaires, avis, plan d'accès. La boulangerie est fictive — mais votre site pourrait ressembler à ça, en partant d'une maquette gratuite."},
  rois:{name:"Rois de France",cat:"Encyclopédie web · PHP/MySQL",img:"assets/shots/rois.jpg",grad:"linear-gradient(135deg,#3a2410,#c8892e 55%,#f2d06b)",role:"Conception, back & front",year:"2025",client:"SAÉ 203 · BUT MMI — noté 18,1/20",tags:["PHP","MySQL","Sécurité XSS/SQL","Lighthouse","VPS"],link:"https://jimmybomy.fr/projets/rois/",desc:"Encyclopédie web sur les monarques de France, de Clovis à Napoléon III : 64 fiches dynamiques, quiz, recherche, sécurisation des formulaires (XSS / injections SQL) et audit Lighthouse. PHP/MySQL, déployé sur mon VPS.",metrics:[["64","fiches dynamiques"],["18,1","note /20"],["0","framework"]],study:[{h:"Le brief",p:"SAÉ 203 du BUT MMI : concevoir de A à Z une encyclopédie web dynamique sur les rois de France — base de données, back PHP, front et mise en ligne réelle. L'exigence : pas une maquette, un site qui tient en production."},{h:"Le défi",p:"Gérer 64 souverains (de Clovis à Napoléon III) sans coder une page par roi, et ouvrir des formulaires au public (recherche, quiz) — donc une vraie surface d'attaque à sécuriser."},{h:"Ce que j'ai fait",p:"Un modèle de données MySQL et des pages PHP paramétrées qui génèrent chaque fiche dynamiquement. Autour du catalogue : filtres par dynastie, recherche et tri, une page de statistiques, un comparateur de règnes, un quiz, un mode aléatoire et même un chatbot — plus un espace admin (back-office) pour gérer le contenu. Côté sécurité : requêtes préparées (PDO) contre l'injection SQL, échappement en sortie contre le XSS, validation des entrées."},{h:"Le résultat",p:"Audité avec Lighthouse, déployé et administré sur mon propre VPS (Apache/PHP/MySQL), noté 18,1/20. Ce que ça prouve : je sais modéliser une base, écrire du PHP sécurisé et livrer moi-même en production."}]},
...
function content(app){
  ...
  if(app.startsWith("project:")){const p=ALL[app.split(":")[1]];
    return `<div class="proj-hero" style="${p.img?`background-image:url('${p.img}');background-size:cover;background-position:top center`:`background:${p.grad}`}">${p.img?"":'<div class="mock"></div>'}</div><div class="h-eyebrow">${p.cat}</div><div class="h-title">${p.name}</div>
      ...
      ${p.metrics?`<div class="pmetrics">${p.metrics.map(m=>`<div class="pm"><b>${m[0]}</b><span>${m[1]}</span></div>`).join("")}</div>`:""}
      ${p.study?`<div class="study">${p.study.map(s=>`<div class="study-b"><div class="study-h">${s.h}</div><p>${s.p}</p></div>`).join("")}</div>`:""}
      ${p.link?(/jimmybomy\.fr/.test(p.link)||p.link.startsWith("/")?`<button class="btn" data-site="${p.link}" data-title="${esc(p.name)}">Ouvrir le projet <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 17L17 7M9 7h8v8"/></svg></button>`:`<a class="btn" href="${p.link}" target="_blank" rel="noopener">Voir le site <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 17L17 7M9 7h8v8"/></svg></a>`):`<span class="btn ghost" style="cursor:default">En développement</span>`}`;}
  if(app==="apropos")return `<div style="display:flex;gap:20px;align-items:center;margin-bottom:6px"><img src="assets/jimmy.jpg?v=2" alt="Jimmy Bomy" style="width:100px;height:126px;object-fit:cover;object-position:top center;border-radius:12px;border:1px solid var(--glass-brd);flex:0 0 auto"><div><div class="h-eyebrow">À propos</div><div class="h-title" style="font-size:clamp(30px,4.2vw,46px)">Salut,<br>moi c'est Jimmy.</div></div></div>
  ...
  if(app==="services")return `<div class="h-eyebrow">Pour les commerces & indépendants</div><div class="h-title">Un site pro,<br>sans prise de tête.</div>
    ...
    <button class="btn" data-site="https://jimmybomy.fr/projets/demo-boulangerie/" data-title="Exemple — La Fournée de Sedan" style="margin-top:18px">Voir un exemple de site →</button>
...
function openSite(url,title){
  ...
  // ouverture directe en vrai plein écran (déclenché par le clic utilisateur)
  if(innerWidth>720){try{const rq=el.requestFullscreen||el.webkitRequestFullscreen;if(rq){const pr=rq.call(el);if(pr&&pr.catch)pr.catch(()=>{});}}catch(e){}}
}
